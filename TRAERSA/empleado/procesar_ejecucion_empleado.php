...
function prepararOFallar($conn, string $sql) {
    ...
    if (!$stmt) {
        ...
        echo json_encode(['success' => false, 'message' => 'Error interno al preparar la consulta. Revisa que se haya ejecutado cambios_base_datos_empleado.sql.']);
        exit;
    }
    return $stmt;
}

$stmtTipo = prepararOFallar($conn, "SELECT tipo_empleado FROM usuarios WHERE id = ?");
...
if ($tipoEmpleado !== 'oficinista') {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'Solo un oficinista puede realizar esta acción.']);
    exit;
}
...
if ($accion === 'asignar') {
    ...
    $stmtEnvio = prepararOFallar($conn, "SELECT estado FROM envios WHERE id_envio = ?");
    ...
    $stmtCond = prepararOFallar($conn, "SELECT id_conductor FROM conductores WHERE usuario_id = ?");
    ...
    if ($filaCond) {
        ...
        $insertCond = prepararOFallar($conn, "INSERT INTO conductores (usuario_id) VALUES (?)");
    ...
    $log = prepararOFallar($conn, "INSERT INTO seguimiento_envio (envio_id, estado, observaciones, fecha) VALUES (?, ?, 'Conductor y vehículo asignados', NOW())");
...
if ($accion === 'estado') {
    ...
    $log = prepararOFallar($conn, "INSERT INTO seguimiento_envio (envio_id, estado, ubicacion, observaciones, fecha) VALUES (?, ?, ?, ?, NOW())");
...
if ($accion === 'agregar_vehiculo') {
    ...
    $check = prepararOFallar($conn, "SELECT id_vehiculo FROM vehiculos WHERE placa = ?");
    ...
    $insert = prepararOFallar($conn, "INSERT INTO vehiculos (placa, tipo, capacidad_kg, estado) VALUES (?, ?, ?, 'Disponible')");
