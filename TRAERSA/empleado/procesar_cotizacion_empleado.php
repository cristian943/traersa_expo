...
if ($accion === 'crear') {
    ...
    if ($clienteId > 0) {
        $stmtCliente = prepararOFallar($conn, "SELECT nombre_empresa, telefono FROM clientes WHERE id_cliente = ?");
    ...
    $update = prepararOFallar($conn, "UPDATE envios SET numero_guia = ? WHERE id_envio = ?");
    $update->bind_param("si", $numeroGuia, $nuevoId);
    $update->execute();

    // Primer registro en la bitácora de seguimiento
    $log = prepararOFallar($conn, "INSERT INTO seguimiento_envio (envio_id, estado, observaciones, fecha) VALUES (?, 'Pendiente', 'Cotización creada', NOW())");
...
if ($accion === 'eliminar') {
    ...
    $stmt = prepararOFallar($conn, "SELECT estado FROM envios WHERE id_envio = ?");
    ...
    $delete = prepararOFallar($conn, "DELETE FROM envios WHERE id_envio = ?");
...
if ($accion === 'estado') {
    ...
    $log = prepararOFallar($conn, "INSERT INTO seguimiento_envio (envio_id, estado, observaciones, fecha) VALUES (?, ?, 'Cambio de estado desde Cotizaciones', NOW())");
